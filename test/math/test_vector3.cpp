...
auto vec_str(const Vector3& vec) -> std::string {
...
auto test_constructors() -> bool {
    ...
    gkit::test::assert_if(vec2.x == 1.0f && vec2.y == 2.0f && vec2.z == 3.0f, "copy constructor failed");
    gkit::test::assert_if(vec3.x == 1.0f && vec3.y == 2.0f && vec3.z == 3.0f, "copy assignment failed");
    return true;
}
...
auto test_arithmetic() -> bool {
    ...
    auto vec4 = vec2 + vec3;
    gkit::test::assert_if(vec4.x == 2.0f && vec4.y == 4.0f && vec4.z == 6.0f, "vec2 + vec3 failed");
    gkit::test::logln("  vec4(vec2 + vec3): {}", vec_str(vec4));

    auto vec5 = vec2 - vec3;
    gkit::test::assert_if(vec5.x == 0.0f && vec5.y == 0.0f && vec5.z == 0.0f, "vec2 - vec3 failed");
    gkit::test::logln("  vec5(vec2 - vec3): {}", vec_str(vec5));

    auto vec6 = vec2 * 10;
    gkit::test::assert_if(vec6.x == 10.0f && vec6.y == 20.0f && vec6.z == 30.0f, "vec2 * 10 failed");
    gkit::test::logln("  vec6(vec2 * 10): {}", vec_str(vec6));

    auto vec7 = vec2 / 5;
    gkit::test::assert_if(vec7.x == 0.2f && vec7.y == 0.4f && vec7.z == 0.6f, "vec2 / 5 failed");
...
auto test_length_and_normalize() -> bool {
    ...
    vec1 = vec1.normalize();
    gkit::test::logln("  normalize(vec1): {}", vec_str(vec1));
